...
@showprogress 1 "MCMC progress..." for iter in 1:chainLength
		...
		for (ySet,yModel) in modelInformation
			...
			if iter in these2Keep
				...
				[[[inOut.outMCMC(outPut,"beta$m$eSet",beta[M[mSet].pos][[p],:]) for (p,m) in enumerate(mSet)] for mSet in keys(M) if (isa(mSet,Tuple{Vararg{Tuple{Vararg{Symbol}}}}) && in(mSet,keys(modelInformation[eSet])))] for eSet in keys(E) if isa(eSet,Symbol)] #multi-trait multiple comp	
