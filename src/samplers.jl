...
@showprogress 1 "MCMC progress..." for iter in 1:chainLength
		...
		for (ySet,yModel) in modelInformation
			...
			[sampleX!(xSet,X,b,ycorr,varE,ySet) for xSet in keys(yModel) if isa(yModel[xSet],FixedEffect) && isa(eSet,Symbol)]
...
#				println("running $(M[mSet].method) for $mSet")
			...
			if iter in these2Keep
				inOut.outMCMC(outPut,"b_$ySet",b') ### currently no path is provided!!!!
