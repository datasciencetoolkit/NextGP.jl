...
function MMEM!(M,beta,varBeta,delta,posMcounter,eSet::Tuple,E,priorVCV,modelInformation,summaryStat)
	...
	correlate = hcat(filter!(!isempty, unique([[keya for (keya,valuea) in priorVCV if ((isa(keya,Tuple) || isa(valuea.v,Matrix{Float64})) && all([in(keyz,keyi) for keyi in keya]))] for keyz in keys(M)]))...)
