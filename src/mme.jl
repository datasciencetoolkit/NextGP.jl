...
function MMEX!(X,b,posXcounter,eSet::Symbol,E,blocks,modelInformation,summaryStat)
	...
	blockX!(X,eSet,blocks,modelInformation)
		...
		if isa(X[xSet][:xpx],Matrix{Float64})
			#println("diag: $(diag(X[xSet][:xpx])) added to diag: $(minimum(abs.(diag(X[xSet][:xpx])./10000)))")
			X[xSet][:xpx] += Matrix(I*minimum(abs.(diag(X[xSet][:xpx])./10000)),size(X[xSet][:xpx]))
			#X[xSet][:xpx] = diagm(diag(X[xSet][:xpx]))
		end
...
#	end	
	...
	[[inOut.outMCMC(outPut,"varU$zSet",[String(zSet)]) for zSet in keys(Z) if (isa(zSet,Union{Symbol,Expr}) && in(zSet,keys(modelInformation[eSet])))] for eSet in keys(E) if isa(eSet,Symbol)] #single trait	
	...
	#c for BayesLV
	[[inOut.outMCMC(outPut,"c$mSet$eSet",hcat(["c$l" for l in 1:length(M[mSet][:c])]...)) for mSet in keys(M) if (isa(mSet,Symbol) && in(mSet,keys(modelInformation[eSet])))] for eSet in keys(E) if isa(eSet,Symbol)] #single trait
	...
	for mSet in keys(varBeta)
		if isa(mSet, Symbol)
			nameM_VCV = ["reg_$r" for r in 1:M[mSet][:nVarCov]]
		...
		elseif isa(mSet, Tuple)
			...
			#nameM_Scale = vcat([["scale_$(i)_$j" for j in 1:length(mSet)^2] for i in 1:M[mSet][:nVarCov]]...)
			nameM_Scale = vcat([["scale_$(i)_$j" for j in 1:length(mSet)^2] for i in 1:1]...) #currently one scale is estimated
			inOut.outMCMC(outPut,"scale_"*join(mSet, "_"),[nameM_Scale]) #[] currently one scale is estimated
			inOut.outMCMC(outPut,"df_"*join(mSet, "_"),["df"]) #[] to have it as one row
		end
		...
		#inOut.outMCMC(outPut,"var$mSet",[nameM_VCV]) #[] to have it as one row
		####
		#isa(mSet, Symbol) ? nameM_VCV = "scale" : nameM_VCV = ["scale$i" for i in 1:M[mSet][:nVarCov]] #size(vec(varBeta[mSet]))
	...
	[inOut.outMCMC(outPut,"varE_"*join(eSet, "_"),hcat(["$(c[1])_$(c[2])" for c in collect(Iterators.product(1:length(eSet), 1:length(eSet)))]...)) for eSet in keys(E) if isa(eSet,Tuple{Vararg{Symbol}})]
