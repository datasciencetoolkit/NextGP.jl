...
#	end	
	...
	[[inOut.outMCMC(outPut,"u$zSet$eSet",[Z[zSet][:levels]]) for zSet in keys(Z) if (isa(zSet,Union{Symbol,Expr}) && in(zSet,modelInformation[eSet]))] for eSet in keys(E) if isa(eSet,Symbol)] #single trait	
	[[[inOut.outMCMC(outPut,"u$zSet$eSet",[Z[zSet][:levels]]) for z in zSet] for zSet in keys(Z) if (isa(zSet,Tuple) && in(zSet,modelInformation[eSet]))] for eSet in keys(E) if isa(eSet,Tuple)] #single-trait multiple comp
