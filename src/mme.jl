...
#	end	
	...
	#c for BayesLV
	[[inOut.outMCMC(outPut,"c$mSet$eSet",["c$l" for l in 1:length(M[mSet][:c])]) for mSet in keys(M) if (isa(mSet,Symbol) && in(mSet,keys(modelInformation[eSet])))] for eSet in keys(E) if isa(eSet,Symbol)] #single trait
