...
#set up (co)variance structures for U
#multi-trait
function setVarCovStrU!(eSet::Tuple{Vararg{Symbol}},zSet::Tuple{Vararg{Tuple{Vararg{Symbol}}}},Z::OrderedDict,priorVCV,varU::Dict)
...
function varCovZ!(Z,priorVCV)
	for zSet ∈ keys(Z)
		#df
		Z[zSet][:df] = 3.0+size(priorVCV[zSet].v,1) - 4.0 ###########################remove this!!!!
		#scale
	...
	#But in general, all models get same prior anyways.
		if haskey(priorVCV,mSet)
			...
			if priorVCV[mSet].params==false
				...
				M[mSet][:params] = false
                		M[mSet][:scale] = nMComp>1 ? priorVCV[mSet].v .* (M[mSet][:df]-nMComp-1.0)  : priorVCV[mSet].v * (M[mSet][:df]-2.0)/(M[mSet][:df])  
			elseif priorVCV[mSet].params==true
				...
				M[mSet][:priorScale] = inv(M[mSet][:scale][])
			end
		else
			M[mSet][:params] = false
			nMComp = 1
			M[mSet][:scale] = 0.05 * (M[mSet][:df]-2.0)/(M[mSet][:df]) 
		end	
	end
end
