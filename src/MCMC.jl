...
"""
runLMEM = function(formula,nChain,nBurn,nThin;myHints=Dict{Symbol,Any}(),blockThese=[],outFolder="outMCMC",VCV=[],userPedData=[],summaryStat=Dict{Any,Any}())
	
	folderHandler(outFolder)

	yVec,X,Z,M = prepMatVec.prep(formula,path2ped=userPedData,priorVCV=VCV)
