...
function prep(f;path2ped=[],priorVCV=[]) ### THE REST OF THE CODE FOR XZM SHOUld also come here, otherwise input data is only the last one in the memory!
	...
	Ainv = []
		...
		elseif isa(v,PedigreeTerm)
			...
			push!(summarize,hcat([[k,typeof(values[k]),typeof(thisZ),size(thisZ,2)] for (keys,values) in modelInformation]...))
			thisZ = 0                
        else    
			X[k] = designMat(k,v,inputData)
			push!(summarize,hcat([[k,typeof(values[k]),typeof(X[k][:data]),X[k][:nCol]] for (keys,values) in modelInformation]...))
            end
        end
