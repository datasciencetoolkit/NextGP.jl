	...
	for (r,theseLoci) in enumerate(M[mSet].regionArray) #theseLoci is always as 1:1,2:2 for BayesB
		for locus in theseLoci::UnitRange{Int64}
			...
			for v in 1:M[mSet].nVarCov
				...
				logLc = varc[v]==0.0 ? -0.5*(log(getindex(M[mSet].mpm,locus)*varE) + (rhs^2)/(getindex(M[mSet].mpm,locus)*varE)) + M[mSet].logPi[v] : -0.5*(log(varc[v]*lhs[v]/varE)-((rhs^2)/(varE*lhs[v]))) + M[mSet].logPi[v]
